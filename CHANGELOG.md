...
- **Slideshow images render fully under tmux, without the flash**: Ghostty through 1.3.1 renders placeholder images delivered as an in-place pane update blank or clipped -- with the data confirmed present in the terminal via the reply channel -- while a full repaint out of tmux's own grid renders them correctly. That is why switching tmux panes completes an image that came out partial. Every draw under a lone tmux layer now ends with `tmux refresh-client`: the same client repaint a pane switch triggers, re-sending nothing (the image is already in the terminal's store, the placeholder cells already in tmux's grid) and costing nothing visible. It replaces the alternate-screen rebuild 0.1.6 used for the same purpose, so slide changes no longer flash the pane underneath, nor pay the 250ms settle that rebuild needed. The repaint is issued after the arrival check, never before, since tmux drops passthrough while a redraw is pending and would eat the question with it; the draws that skip that check -- animations, and the message slide shown for a file that will not load -- wait the same interval before the repaint instead, since they carry passthrough of their own. Where a repaint cannot reach -- screen, which has no equivalent command, and tmux nested in another multiplexer, whose outer layer would receive the repaint as one more in-place update -- slide changes still rebuild by way of the pane's main screen; that rebuild remains the last-attempt fallback when transmissions are being lost, and what the `r` key forces by hand.
- `examples/placeholder_probe` gained a second group of variants (`manual`, `refresh`, `nudge`, `rewrite`, `cycle-refresh`) that bisect the *repair* rather than the draw: given a slide that rendered wrong, which nudge completes it. On Ghostty 1.3.1 + tmux only the two that force a full client repaint do -- unrelated terminal traffic does not, and re-sending the placeholder cells in place does not -- which is what settled the fix above.
- `test/slideshow-test.sh` now checks for the absence of the inter-slide flash it previously told the tester to expect, and asks directly whether any slide came out clipped.
